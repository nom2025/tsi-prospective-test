...
| 項目 | 仕様 |
|---|---|
| データソース | 国土地理院 GNSS日座標（〜2026-03-31: F5解 / 2026-04-01〜: F5.1解） |
| 観測点数 | 7局（固定） |
...
|---|---|
| 2026-02-14 | 初版作成・凍結 |
| 2026-04-01 | データソース移行: 国土地理院 F5解（ITRF2014）→ F5.1解（ITRF2020）。国土地理院の提供終了に伴う不可避の変更。TSI計算は基準日からの相対変動量（delta_x/y/z）を使用するため、絶対測地基準系の変更は計算式・凍結パラメータ（mean/std・閾値）に影響しない。前向き検証は継続有効。 |
| 2026-05-31 | 2026-05-24 局ID誤り判明により前向き検証を中止。GitHub Actions スケジュール停止（`.github/workflows/daily-tsi.yml` の cron 無効化、手動実行のみ）。 |
